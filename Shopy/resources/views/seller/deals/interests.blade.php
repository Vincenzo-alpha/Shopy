@extends('layouts.seller')

@section('title', 'Customer Interests - Seller Portal')
@section('page_title', 'Incoming Customer Interests')

@section('content')
<div class="card border-0 shadow-sm overflow-hidden">
    <div class="card-header bg-white py-3">
        <h6 class="fw-bold mb-0">Buyer Interest Requests</h6>
        <small class="text-muted">Initiate a deal to start direct price negotiation with the customer</small>
    </div>
    <div class="table-responsive">
        <table class="table table-hover align-middle mb-0">
            <thead class="table-light">
                <tr>
                    <th class="ps-4">Interest Ref</th>
                    <th>Customer</th>
                    <th>Interested Item</th>
                    <th>Listed Price</th>
                    <th>Status</th>
                    <th>Received At</th>
                    <th class="text-end pe-4">Action</th>
                </tr>
            </thead>
            <tbody>
                @forelse($interests as $interest)
                    <tr>
                        <td class="ps-4 font-monospace small text-primary fw-semibold">
                            {{ $interest->interest_unique_no }}
                        </td>
                        <td>
                            <div class="fw-bold">{{ $interest->customer->customer_name }}</div>
                            <small class="text-muted"><i class="bi bi-geo-alt"></i> {{ $interest->customer->city }}</small>
                        </td>
                        <td>
                            <div class="fw-bold">{{ $interest->product->prod_service_name }}</div>
                            <span class="badge {{ $interest->product->item_type === 'service' ? 'badge-type-service' : 'badge-type-product' }} small">
                                {{ ucfirst($interest->product->item_type) }}
                            </span>
                        </td>
                        <td class="fw-bold text-dark">₹{{ number_format($interest->product->listed_price, 2) }}</td>
                        <td>
                            @if($interest->interest_status === 'Active')
                                <span class="badge bg-success-subtle text-success border border-success px-2 py-1">New Interest</span>
                            @elseif($interest->interest_status === 'Deal Denied' || ($interest->deal && $interest->deal->deal_status === 'cancelled'))
                                <span class="badge bg-danger-subtle text-danger border border-danger-subtle px-2 py-1">Deal Denied</span>
                            @elseif($interest->interest_status === 'Converted to deal')
                                <span class="badge bg-primary-subtle text-primary border border-primary px-2 py-1">Deal Initiated</span>
                            @else
                                <span class="badge bg-secondary-subtle text-secondary px-2 py-1">Withdrawn</span>
                            @endif
                        </td>
                        <td class="small text-muted">{{ $interest->created_at->format('M d, Y h:i A') }}</td>
                        <td class="text-end pe-4">
                            @if($interest->interest_status === 'Active')
                                <form action="{{ route('seller.deals.initiate', $interest->interest_id_pk) }}" method="POST" class="d-inline">
                                    @csrf
                                    <button type="submit" class="btn btn-primary btn-sm fw-bold">
                                        <i class="bi bi-chat-dots me-1"></i> Initiate Deal
                                    </button>
                                </form>
                            @elseif($interest->deal)
                                <a href="{{ route('seller.deals.show', $interest->deal->deal_id_pk) }}" class="btn btn-outline-primary btn-sm">
                                    <i class="bi bi-arrow-right-circle me-1"></i> Open Deal
                                </a>
                            @else
                                <span class="text-muted small">No action</span>
                            @endif
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="7" class="text-center py-5 text-muted">
                            <i class="bi bi-bell-slash display-4 opacity-25 d-block mb-3"></i>
                            <h6 class="fw-bold">No customer interests received yet</h6>
                            <p class="small mb-0">When buyers express interest in your marketplace listings, they will appear here.</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    </div>
    @if($interests->hasPages())
        <div class="card-footer bg-white border-0 py-3 d-flex justify-content-center">
            {{ $interests->links('pagination::bootstrap-5') }}
        </div>
    @endif
</div>
@endsection
